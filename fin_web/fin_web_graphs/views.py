...
class GraphMovingAverage(TemplateView):
    template_name = 'fin_web_graphs/graph.html'

    def get_context_data(self, **kwargs):
        data = Olhc.objects.all()

        context = super(GraphMovingAverage, self).get_context_data(**kwargs)
        x = []
        y = []
...
class GraphWeekMovingAverage(TemplateView):
    template_name = 'fin_web_graphs/graph.html'

    def get_context_data(self, **kwargs):
        data = Olhc.objects.all()

        context = super(GraphWeekMovingAverage, self).get_context_data(**kwargs)
        x = []
        y = []
        ...
        dates = pd.date_range(end=df.iloc[0]['date'], periods=shift, freq='D')
        values = np.sort(np.random.randint(df[:]['value'].min(), df[:100]['value'].max(), len(dates)))

        df2 = pd.DataFrame({"date": dates.date, "value": values})
        df = df.append(df2)
        df.sort_values(by=['date'], inplace=True)

        df['WMA'] = df['value'].rolling(window=shift).mean()
        df = df.iloc[shift:]
...
class GraphStockToFlow(TemplateView):
    template_name = 'fin_web_graphs/graph.html'

    def get_context_data(self, **kwargs):
        data = Olhc.objects.all()

        context = super(GraphStockToFlow, self).get_context_data(**kwargs)
        date = []
        ...
        trace3 = go.Scatter(x=df['date'], y=df['low'], marker_color='rgba(255, 165, 0, .8)', mode="lines")
